...
class ActiveGui(qtw.QWidget):
    ...
    def load_mpv_tab(self, parent: qtw.QWidget):
        ...
        kp.setPlaceholderText("kp")
        layout.addRow("kp", kp)
        ki = qtw.QLineEdit()
        ki.setValidator(qtg.QDoubleValidator())
        ki.setAlignment(Qt.AlignCenter)
        ki.setPlaceholderText("ki")
        layout.addRow("ki", ki)
        kd = qtw.QLineEdit()
        kd.setValidator(qtg.QDoubleValidator())
        kd.setAlignment(Qt.AlignCenter)
        kd.setPlaceholderText("kd")
        layout.addRow("kd", kd)
        target = qtw.QLineEdit()
        target.setValidator(qtg.QDoubleValidator())
        target.setAlignment(Qt.AlignCenter)
        layout.addRow("target", target)
        if self.pid_controller:
            kp.setText(str(self.pid_controller.tunings[0]))
            ki.setText(str(self.pid_controller.tunings[1]))
            kd.setText(str(self.pid_controller.tunings[2]))
            target.setText(str(self.pid_controller.target))
        apply_btn = qtw.QPushButton("Apply")
        ...
        def apply_settings():
            try:
                tunings = (float(GuiHelper.get_data_from_widget(kp)),
                           float(GuiHelper.get_data_from_widget(ki)),
                           float(GuiHelper.get_data_from_widget(kd)))
                setpoint_val = float(GuiHelper.get_data_from_widget(target))
